...
public class ExtractInformationHolderRefactoring extends AbstractMDSLGenerator {
			...
			if(endpoint.getName().equals(sourceEndpoint)) {
				for(io.mdsl.apiDescription.Operation operation : endpoint.getOps()) {
					if(operation.getName().equals(sourceOperation)) {
						...
						MessageTransformations.extractInformationHolder(operation, true); // TODO also apply to response		
						CharSequence result = "// Interface refactoring 'Extract Information Holder' applied.\n";
