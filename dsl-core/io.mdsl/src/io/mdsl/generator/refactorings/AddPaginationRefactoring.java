...
public class AddPaginationRefactoring extends AbstractMDSLGenerator {

	public AddPaginationRefactoring(String sourceEndpoint, String sourceOperation, String variant) {
		super();
	...
	private String variant = "pageFromOperation";
			...
			if(endpoint.getName().equals(sourceEndpoint)) {
				for(io.mdsl.apiDescription.Operation operation : endpoint.getOps()) {
					if(operation.getName().equals(sourceOperation)) {
						...
						MessageTransformations.addPagination(es, variant);						
						CharSequence result = "// Interface refactoring 'Add Pagination' applied.\n";
