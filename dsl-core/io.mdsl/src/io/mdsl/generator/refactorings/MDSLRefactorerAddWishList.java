...
public class MDSLRefactorerAddWishList extends AbstractMDSLGenerator {

	public MDSLRefactorerAddWishList(String sourceEndpoint, String sourceOperation, String targetEndpoint) {
		super();
			...
			if(endpoint.getName().equals(sourceEndpoint)) {
				for(io.mdsl.apiDescription.Operation operation : endpoint.getOps()) {
					if(operation.getName().equals(sourceOperation)) {
						...
						MessageTransformations.addWishList(es, "*");						
						CharSequence result = "// Interface refactoring 'Add WishList' done:\n";
