...
public class InlineInformationHolderRefactoring extends AbstractMDSLGenerator {

	public InlineInformationHolderRefactoring(String sourceEndpoint, String sourceOperation) {
		super();
			...
			if(endpoint.getName().equals(sourceEndpoint)) {
				for(io.mdsl.apiDescription.Operation operation : endpoint.getOps()) {
					if(operation.getName().equals(sourceOperation)) {
						...
						MessageTransformations.inlineInformationHolder(operation, true); // TODO also apply to response		
						CharSequence result = "// Interface refactoring 'Inline Information Holder' applied.\n";
