...
public class AddHttpResourceForURITemplateRefactoring extends AbstractMDSLGenerator {

	public AddHttpResourceForURITemplateRefactoring(String sourceEndpoint, String sourceOperation, String template) {
		super();
			...
			if(endpoint.getName().equals(sourceEndpoint)) {
				for(io.mdsl.apiDescription.Operation operation : endpoint.getOps()) {
					if(operation.getName().equals(sourceOperation)) {
						...
						// TODO Eclipse QF gets {uritemplate} from validation message, which is not possible here. transformation should find the value 
						hbt.addHttpResourceForURITemplate(opB, template); 
						CharSequence result = "// Interface refactoring 'AddHttpResourceForURITemplateRefactoring' applied.\n";
