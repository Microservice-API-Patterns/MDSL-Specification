...
// generated from orchestration flow part of flowtest4b-allbranchingoptions.mdsl
// The API description FlowTest4 features 2 orchestration flow(s) (a.k.a. API call sequences).
...
public class AllAndOptionsInMDSLWithOverlap extends RouteBuilder {
	class FlowInitiated {} 
    ...
    public static void main(String[] args) throws Exception {
        AllAndOptionsInMDSLWithOverlap builder = new AllAndOptionsInMDSLWithOverlap();
        builder.runAllAndOptionsInMDSLWithOverlap();
    }
    
    public void runAllAndOptionsInMDSLWithOverlap() throws Exception {
        // create CamelContext
        ...
        String testMessage = "Test message for flow AllAndOptionsInMDSLWithOverlap in FlowTest4";
    ...
    public void configure() {
        ...
        from("direct:AND_FlowStep3Completed_FlowStep4Completed:AggregatorIn").aggregate(new JoinAggregatorStrategy()).constant(true).completionSize(2).to("direct:FlowStep5");
        ...
        from("direct:FlowStep5").process(new FlowStep5Processor("FlowStep5")).to("direct:FlowTerminated"); // command to single event
...
public class AllOrOptionsInMDSLWithOverlap extends RouteBuilder {
	class FlowInitiated {} 
	class FlowStep1OutEvent1 {} 
	class FlowStep1OutEvent2 {} 
	class FlowStep2Completed {} 
	class FlowStep3Completed {} 
	class FlowStep4Completed {} 
	class AND_FlowStep2Completed_FlowStep3Completed_FlowStep4Completed {} 
	class FlowTerminated {} 
    ...
    public static void main(String[] args) throws Exception {
        AllOrOptionsInMDSLWithOverlap builder = new AllOrOptionsInMDSLWithOverlap();
        builder.runAllOrOptionsInMDSLWithOverlap();
    }
    
    public void runAllOrOptionsInMDSLWithOverlap() throws Exception {
        // create CamelContext
        DefaultCamelContext camelContext = new DefaultCamelContext();
        ...
        // create a producer to send all initial events to the process flow
        ProducerTemplate template = camelContext.createProducerTemplate();
        String testMessage = "Test message for flow AllOrOptionsInMDSLWithOverlap in FlowTest4";
		template.sendBody("direct:FlowInitiated", testMessage);
        // TODO add header to message if flow contains a choice, example (replace "choice-nn" with names from flow):
        // template.sendBodyAndHeader("direct:FlowInitiated", testMessage, "ChoiceCondition", "choiceValue"); 
        camelContext.stop();
    ...
    public void configure() {
        // routes for command invocation steps
        from("direct:FlowInitiated").choice() // event to multiple commands 
        .when(simple("${header.FlowInitiatedCommandInvocationCondition} == 'choiceOfFlowStep1'")).to("direct:FlowStep1") 
        .when(simple("${header.FlowInitiatedCommandInvocationCondition} == 'choiceOfFlowStep2'")).to("direct:FlowStep2")
        ...
        // note: FlowStep3Completed does not invoke any commands 
        // note: FlowStep4Completed does not invoke any commands 
        from("direct:FlowStep2Completed").to("direct:AND_FlowStep2Completed_FlowStep3Completed_FlowStep4Completed:AggregatorIn");
        from("direct:FlowStep3Completed").to("direct:AND_FlowStep2Completed_FlowStep3Completed_FlowStep4Completed:AggregatorIn");
        from("direct:FlowStep4Completed").to("direct:AND_FlowStep2Completed_FlowStep3Completed_FlowStep4Completed:AggregatorIn");
        from("direct:AND_FlowStep2Completed_FlowStep3Completed_FlowStep4Completed:AggregatorIn").aggregate(new JoinAggregatorStrategy()).constant(true).completionSize(3).to("direct:FlowStep5");
        // note: FlowTerminated does not invoke any commands 

        // routes for domain event production steps
        from("direct:FlowStep1").choice() // command to multiple events 
        .when(simple("${header.FlowStep1EventEmissionCondition} == 'choiceOfFlowStep1OutEvent1'")).to("direct:FlowStep1OutEvent1")
        .when(simple("${header.FlowStep1EventEmissionCondition} == 'choiceOfFlowStep1OutEvent2'")).to("direct:FlowStep1OutEvent2")
        .otherwise().to("mock:bye").stop();
        from("direct:FlowStep2").process(new FlowStep2Processor("FlowStep2")).to("direct:FlowStep2Completed"); // command to single event
        from("direct:FlowStep3").process(new FlowStep3Processor("FlowStep3")).to("direct:FlowStep3Completed"); // command to single event
        from("direct:FlowStep4").process(new FlowStep4Processor("FlowStep4")).to("direct:FlowStep4Completed"); // command to single event
